...
public:
    ...
    snapshot_registry() : core_(std::make_shared<registry_core>()) {}
    ...
    [[nodiscard]] subscription subscribe(callback_type callback) {
        ...
        return core_->subscribe(std::move(callback));
    }
    ...
    [[nodiscard]] std::size_t active_count() const {
        ...
        return static_cast<std::size_t>(std::count_if(
            current->begin(), current->end(), [](const subscriber_ptr& subscriber) {
...
private:
    struct registry_core final : std::enable_shared_from_this<registry_core> {
        registry_core() : current(std::make_shared<const subscriber_list>()) {}
        ...
        [[nodiscard]] subscription subscribe(callback_type callback) {
            ...
            auto state = std::make_shared<subscriber_type>(
                id, std::move(callback), [weak_core, id] {
        ...
        void retire(detail::subscriber_id id) {
            ...
            const auto found = std::find_if(
                previous->begin(), previous->end(), [id](const subscriber_ptr& subscriber) {
